...
echo -e "${YELLOW}已备份 app.py 为 app.py.bak${NC}"
...
APP_CFPORT=$(grep "CFPORT = " app.py | cut -d"'" -f2)
...
echo -e "优选端口: ${APP_CFPORT}"
echo -e "${YELLOW}========================${NC}"
echo

# 添加YouTube分流和80端口节点补丁（文件操作使用内置读取减少fork）
...
echo -e "服务端口: ${BLUE}$APP_PORT${NC}"
echo -e "UUID: ${BLUE}$APP_UUID${NC}"
echo -e "${YELLOW}============================${NC}"
echo
...
if command -v curl &> /dev/null; then
    ...
    SAVE_INFO="${SAVE_INFO}
订阅地址: http://$PUBLIC_IP:$APP_PORT/$SUB_PATH
管理面板: http://$PUBLIC_IP:$APP_PORT"
fi
SAVE_INFO="${SAVE_INFO}
本地订阅: http://localhost:$APP_PORT/$SUB_PATH
本地面板: http://localhost:$APP_PORT
